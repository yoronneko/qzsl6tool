...
# -*- coding: utf-8 -*-
#
# libbit.py: bit opereation library for GNSS data processing
...
def setbitu (buff, pos, length, data):
    mask = 1 << (length - 1)
    if length <= 0 or 32 < length: return
    for i in range (pos, pos + length):
        if data & mask: buff[i // 8] |=   1 << (7 - i % 8)
        else          : buff[i // 8] &= ~(1 << (7 - i % 8))
        mask >>= 1

def setbits (buff, pos, length, data):
    if data < 0: data |=   1 << (length - 1)
    else       : data &= ~(1 << (length - 1))
