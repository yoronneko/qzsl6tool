# QZS L6 Tool: quasi-zenith satellite L6-band tool, ver.0.1.9
...
イメージには実行環境とコードのみを含み、Git、ビルドツール、テスト、サンプルは含みません。検証用データはホスト側でcloneしてマウントします。次のコマンドは、イメージ内のPythonコードを検証します。サンプルの実行例は、このリポジトリーのルートで実行してください。
