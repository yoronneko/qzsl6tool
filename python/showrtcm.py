...
class rtcm_t:
    ...
    def receive (self):
        ...
        try:
            ...
            ok = False
            while not ok:
                if bc == rtk_crc24q (frame, len (frame)): ok = True
                else: # CRC error
                    frame = frame[1:] + bc
                    syncpos = -1
    ...
    def mnum2satsys (self, mnum = 0): # message number to satellite system
        ...
        if mnum in {1009, 1010, 1011, 1012, 1020, 1081, 1081, 1082, 1083, \
                    1084, 1085, 1086, 1087, 1063, 1064, 1065, 1066, 1067, 1068}:
           satsys = 'R'
    ...
    def ssr_decode_orbit (self): # decode SSR orbit correction
        ...
        for i in range (self.ssr.nsat):
            ...
            iode   = getbitu (payload, pos,  8); pos +=  8 # IODE
            drad   = getbits (payload, pos, 22); pos += 22 # delta radial
            dalng  = getbits (payload, pos, 20); pos += 20 # delta along track
            dcrs   = getbits (payload, pos, 20); pos += 20 # delta cross track
    ...
    def decode_pos (self): # decode antenna position
        ...
        stid = getbitu (payload, pos, 12); pos += 12
        itrf = getbitu (payload, pos,  6); pos +=  6+4
        px = getbits38 (payload, pos) * 1e-4; pos += 38+2
        py = getbits38 (payload, pos) * 1e-4; pos += 38+2
        pz = getbits38 (payload, pos) * 1e-4; pos += 38
        if self.mnum == 1006:
    ...
    def decode_ant_info (self): # decode antenna and receiver info
        ...
        str_rsn  = ''
        stid = getbitu (payload, pos, 12); pos += 12
        l    = getbitu (payload, pos,  8); pos +=  8
        for i in range (l):
            str_ant += chr (getbitu (payload, pos, 8)); pos += 8
        ant_setup = getbitu (payload, pos, 8); pos += 8
        if self.mnum == 1008 or self.mnum == 1033:
            l = getbitu (payload, pos, 8); pos += 8
            if 31 < l: l = 31
            for i in range (l):
                str_ser += chr (getbitu (payload, pos, 8)); pos += 8
        if self.mnum == 1033:
            l = getbitu (payload, pos, 8); pos += 8
            if 31 < l: l = 31
            for i in range (l):
                str_rcv += chr (getbitu (payload, pos, 8)); pos += 8
            l = getbitu (payload, pos, 8); pos += 8
            if 31 < l: l = 31
            for i in range (l):
                str_ver += chr (getbitu (payload, pos, 8)); pos += 8
            l = getbitu (payload, pos, 8); pos += 8
            if 31 < l: l = 31
            for i in range (l):
    ...
    def decode_ephemerides (self):
        ...
        if satsys == 'G': # type 1019 GPS ephemerides
            svid = getbitu (payload, pos, 6); pos += 6
            pos +=10+4+2+14+8+16+8+16+22+10+16+16+32+16+32+16+32+ \
                  16+16+32+16+32+16+32+24+8
            svh = getbitu (payload, pos, 6); pos += 6
            pos += 1+1
        elif satsys == 'R':
            svid = getbitu (payload, pos, 6); pos += 6
            pos += 5+2+2+5+6+1+1+1+7+24+27+5+24+27+5+24+27+5+1+ \
                   11+3+22+5+5
        elif satsys == 'I':
            svid = getbitu (payload, pos, 6); pos += 6
            pos += 10+22+16+8+4+16+8+22+8+10
            svh = getbitu (payload, pos, 2); pos += 2
            pos += 15+15+15+15+15+15+14+32+16+32+32+32+32+22+32
        elif satsys == 'J':
            svid = getbitu (payload, pos, 4); pos += 4
            pos += 16+8+16+22+8+16+16+32+16+32+16+32+16+16+32+ \
                   16+32+16+32+24+14+2+10+4
            svh = getbitu(payload, pos, 6); pos += 6
            pos += 8+10+1
        elif mtype == 'F/NAV': # type 1045 Galileo ephemerides
            svid = getbitu (payload, pos, 6); pos += 6
            pos += 12+10+8+14+14+6+21+31+16+16+32+16+32+16+32+ \
                   14+16+32+16+32+16+32+24+10+2+1+7
        elif mtype == 'I/NAV':
            svid = getbitu (payload, pos, 6); pos += 6
            pos += 12+10+8+14+14+6+21+31+16+16+32+16+32+16+32+ \
                   14+16+32+16+32+16+32+24+10+10+2+1+2+1
        elif satsys == 'C':
            svid = getbitu (payload, pos, 6); pos += 6
            pos += 13+4+14+5+17+11+22+24+5+18+16+32+18+32+18+ \
                   32+17+18+32+18+32+18+32+24+10+10
            svh = getbitu (payload, pos, 1); pos += 1
        #streph = ''
        #leneph = pos - postop
        #while 0 < leneph:
    ...
    def decode_msm (self): # decode MSM message
        ...
        stid    = getbitu (payload, pos, 12); pos += 12
        if satsys == 'R':
            dow = getbitu (payload, pos,  3)        ; pos +=  3
            tod = getbitu (payload, pos, 27) // 1000; pos += 27
        elif satsys == 'C':
            tow = getbitu (payload, pos, 30) // 1000; pos += 30
            tow += 14 # BDT -> GPST
        else:
            tow = getbitu (payload, pos, 30) // 1000; pos += 30
        sync    = getbitu (payload, pos, 1); pos += 1
        iod     = getbitu (payload, pos, 3); pos += 3
        time_s  = getbitu (payload, pos, 7); pos += 7
        clk_s   = getbitu (payload, pos, 2); pos += 2
        cls_e   = getbitu (payload, pos, 2); pos += 2
        smth    = getbitu (payload, pos, 1); pos += 1
        tint_s  = getbitu (payload, pos, 3); pos += 3
        sat_mask = [0 for i in range (64)]; n_sat_mask = 0
        for i in range (64):
            mask = getbitu (payload, pos, 1); pos += 1
            if mask: sat_mask[n_sat_mask] = i; n_sat_mask += 1
        sig_mask = [0 for i in range (32)]; n_sig_mask = 0
        for i in range (32):
            mask = getbitu (payload, pos, 1); pos += 1
            if mask: sig_mask[n_sig_mask] = i; n_sig_mask += 1
        cell_mask = [0 for i in range (n_sat_mask * n_sig_mask)]
        for i in range (n_sat_mask * n_sig_mask):
            cell_mask[i] = getbitu (payload, pos, 1); pos += 1
        for i in range (n_sat_mask): # range
            rng = getbitu (payload, pos, 8); pos += 8
        for i in range (n_sat_mask):
            rng_m = getbitu (payload, pos, 10); pos += 10
        for i in range (n_sat_mask * n_sig_mask): # pseudorange
            if not cell_mask[i]: continue
            prv = getbitu (payload, pos, 15); pos += 15
        lock = []
...
if __name__ == '__main__':
    ...
    while rtcm.receive ():
        ...
        if rtcm.mtype == 'CSSR'               : rtcm.decode_cssr ()
            ...
            elif rtcm.mtype == 'SSR code bias': rtcm.ssr_decode_code_bias ()
            elif rtcm.mtype == 'SSR URA'      : rtcm.ssr_decode_ura ()
            elif rtcm.mtype == 'SSR hr clock' : rtcm.ssr_decode_hr_clock ()
