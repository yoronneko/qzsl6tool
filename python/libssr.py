...
# --- private
    ...
    def ssr_decode_head(self, payload, pos, satsys, mtype):
        ...
        # ssr update interval
        ssr_interval = payload[pos:pos+ 4].uint; pos +=  4
        # multiple message indicator
        self.ssr_mmi = payload[pos:pos+ 1].uint; pos +=  1
    ...
    def ssr_decode_orbit(self, payload, pos, satsys):
        ...
        for i in range(self.ssr_nsat):
            ...
            drad   = payload[pos:pos+22].int ; pos += 22  # delta radial
            dalng  = payload[pos:pos+20].int ; pos += 20  # delta along track
            dcrs   = payload[pos:pos+20].int ; pos += 20  # delta cross track
            ddrad  = payload[pos:pos+21].int ; pos += 21  # delta^2 radial
            ddalng = payload[pos:pos+19].int ; pos += 19  # delta^2 along track
            ddcrs  = payload[pos:pos+19].int ; pos += 19  # delta^2 cross track
    ...
    def ssr_decode_clock(self, payload, pos, satsys):
        ...
        for i in range(self.ssr_nsat):
            satid = payload[pos:pos+bw].uint; pos += bw  # satellite ID
            dc0   = payload[pos:pos+22].int ; pos += 22  # delta clock c0
            dc1   = payload[pos:pos+21].int ; pos += 21  # delta clock c1
            dc2   = payload[pos:pos+27].int ; pos += 27  # delta clock c2
            strsat += f"{satsys}{satid:02} "
        string = f"{strsat}(nsat={self.ssr_nsat} iod={self.ssr_iod}" + \
                      f"{' cont.' if self.ssr_mmi else ''})"
        return pos, string
    ...
    def ssr_decode_code_bias(self, payload, pos, satsys):
        ...
        for i in range(self.ssr_nsat):
            ...
            for j in range(ncb):
                ...
                stmi = payload[pos:pos+ 5].uint; pos +=  5
                cb   = payload[pos:pos+14].int ; pos += 14  # code bias
        string = f"{strsat}(nsat={self.ssr_nsat} iod={self.ssr_iod}" + \
                      f"{' cont.' if self.ssr_mmi else ''})"
        return pos, string
    ...
    def ssr_decode_hr_clock(self, payload, pos, satsys):
        ...
        for i in range(self.ssr_nsat):
            satid = payload[pos:pos+bw].uint; pos += bw  # satellite ID
            hrc   = payload[pos:pos+22].int ; pos += 22  # high rate clock
            strsat += f"{satsys}{satid:02} "
        string = f"{strsat}(nsat={self.ssr_nsat} iod={self.ssr_iod}" + \
                      f"{' cont.' if self.ssr_mmi else ''})"
        return pos, string
    ...
    def decode_cssr_head(self, payload):
        ...
        self.mmi      = payload[pos:pos + 1].uint; pos += 1  # multi msg ind
        ...
        for i, satsys in enumerate(self.satsys):
            ...
            for j, gsys in enumerate(self.gsys[satsys]):
                for k, gsig in enumerate(self.gsig[satsys]):
                    ...
                    if not mask:
                        continue
                    cb = payload[pos:pos + 11].int; pos += 11
    ...
    def decode_cssr_st6(self, payload, pos):
        ...
        if len_payload < 45:
            return 0
        stat_pos = pos
        f_cb = payload[pos:pos + 1].uint; pos += 1  # code bias existing flag
        f_pb = payload[pos:pos + 1].uint; pos += 1  # phase bias existing flag
        f_nb = payload[pos:pos + 1].uint; pos += 1  # network bias existing flag
        svmask = {}
        cnid = 0
        for satsys in self.satsys:
                ...
                for gsig in self.gsig[satsys]:
                    ...
                    if f_cb:
                        if len_payload < pos + 11:
                            return 0
                        cb = payload[pos:pos + 11].int; pos += 11  # code bias
                        code_bias = cb * 0.02 if cb != -1024 else INVALID
        ...
        for satsys in self.satsys:
            for i, gsys in enumerate(self.gsys[satsys]):
                ...
                if len_payload < pos + 6 + 14:
                    return 0
                qi = payload[pos:pos + 6].uint; pos += 6
                # quality indicator
    ...
    def decode_cssr_st9(self, payload, pos):
        ...
        if len_payload < 45:
            return 0
        # tropospheric correction type
        tctype = payload[pos:pos + 2].uint; pos += 2
        # tropospheric correction range
        crange = payload[pos:pos + 1].uint; pos += 1
        bw = 16 if crange else 7
        cnid   = payload[pos:pos + 5].uint; pos += 5
        # compact network ID
        svmask = {}
        for satsys in self.satsys:
            ngsys = len(self.gsys[satsys])
        ...
        if len_payload < pos + 6 + 6:
            return 0
        # tropospheric quality indicator
        tqi   = payload[pos:pos + 6].uint; pos += 6
        ...
        for i in range(ngrid):
            ...
            ivd_h = payload[pos:pos + 9].int; pos += 9
            vd_h = ivd_h * 0.004 if ivd_h != -256 else INVALID
            # wet vertical delay
            ivd_w = payload[pos:pos + 8].int; pos += 8
    ...
    def decode_cssr_st10(self, payload, pos):
        ...
        datasize = (payload[pos:pos+2].uint + 1) * 40; pos += 2  # data size
        if len_payload < pos + datasize:
            return 0
        aux_frame_data = payload[pos:pos+datasize]; pos += datasize
    ...
    def decode_cssr_st11(self, payload, pos):
        ...
        if len_payload < 40:
            return 0
        stat_pos = pos
        f_o = payload[pos:pos + 1].uint; pos += 1  # orbit existing flag
        f_c = payload[pos:pos + 1].uint; pos += 1  # clock existing flag
        f_n = payload[pos:pos + 1].uint; pos += 1  # network correction
        msg_trace1 = \
            ...
            for satsys in self.satsys:
                for i, gsys in enumerate(self.gsys[satsys]):
                    ...
                    if f_o:
                        ...
                        if len_payload < pos + w_iode + 15 + 13 + 13:
                            return 0
                        iode      = payload[pos:pos+w_iode].uint; pos += w_iode
    ...
    def decode_cssr_st12(self, payload, pos):
        ...
        if tropo[1]:
            if len_payload < pos + 1 + 4:
                return 0
            trs  = payload[pos:pos + 1].uint; pos += 1 # tropo residual size
            bw = 8 if trs else 6
            ...
            for satsys in self.satsys:
                for i, gsys in enumerate(self.gsys[satsys]):
                    ...
                    if len_payload < pos + 6 + 2 + 14:
                        return 0
                    sqi  = payload[pos:pos + 6].uint; pos += 6  # quality indi
